...
class WebSocketService {
     ...
     */
    private static getWebSocketMessageResponse<T>(
        type: WebSocketMessageType,
        requestId: string,
        responseCode: ResponseCode,
        body?: any,
    ): WebSocketMessage<T> {
        return new WebSocketMessage<T>(WebSocketMessageKind.RESPONSE, type, body, requestId, responseCode);
    }
    ...
    private static getWebSocketMessageBroadcast<T>(type: WebSocketMessageType, body?: any): WebSocketMessage<T> {
        return new WebSocketMessage<T>(WebSocketMessageKind.BROADCAST, type, body);
    }
     ...
     */
    private static getEmptyWebSocketMessageResponse(
        requestId: string,
        code: ResponseCode,
    ...
    private handleConnectionRequest = (request: WebSocket.request): void => {
        ...
        this.handleClientConnected().then(response => WebSocketService.sendResponse(client, response));

        client.on('message', (message: { type: string; utf8Data: any }) => {
            this.handleMessageReceived(message).then(response => WebSocketService.sendResponse(client, response));
        });
     ...
     */
    private handleMessageReceived = async (message: {
        type: string;
        utf8Data: any;
    }): Promise<WebSocketMessage<any>> => {
    ...
    private handleCommandRequest(request: WebSocketMessage<ICommandRequest>): Promise<{ code: ResponseCode }> {
        return new Promise((resolve, reject) => {
            ...
            if (!board) {
                ...
                this.boardModel
                    .executeActionOnBoard(board.id, command)
                    .catch(error => {
     ...
     */
    private handleBoardRequest(
        request: WebSocketMessage<IBoardRequest>,
    ): Promise<{ body: IBoardResponse | undefined; code: ResponseCode }> {
        return new Promise((resolve, reject) => {
            const result: {
                body: IBoardResponse;
                code: ResponseCode;
    ...
    private handleClientConnected(): Promise<WebSocketMessage<IBoardBroadcast>> {
        return new Promise(resolve => {
            ...
            resolve(
                new WebSocketMessage<IBoardBroadcast>(
                    WebSocketMessageKind.BROADCAST,
                    WebSocketMessageType.BOARD_BROADCAST,
                    body,
                ),
            );
        });
    ...
    private broadcastBoardUpdate = (action: BOARD_BROADCAST_ACTION, board: IBoard): void => {
        ...
        const message = WebSocketService.getWebSocketMessageBroadcast<IBoardBroadcast>(
            WebSocketMessageType.BOARD_BROADCAST,
            body,
        );
