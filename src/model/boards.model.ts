 ...
 * @namespace BoardsModel
 */
class BoardsModel extends events.EventEmitter {
    /**
     ...
     * @type {LoggerService}
     */
    private static log = new LoggerService(BoardsModel.namespace);
     ...
     * Currently supported types are {@link Board} (default) and {@link MajorTom}, as dictated by {@link BoardsModel.AVAILABLE_TYPES}.
     */
    ...
    private static async findOrBuildBoard(id: string, type: string): Promise<Board> {
        ...
        board = BoardsModel.instantiateNewBoard(board);
    ...
    public synchronise(): Promise<void> {
        return Board.findAll().then(boards => {
            this._boards = boards.map(board => BoardsModel.instantiateNewBoard(board));
        });
    }
    ...
    public deleteBoard(boardId: string): void {
        const board = this.getBoardById(boardId);
        BoardsModel.log.debug(`Deleting board with id ${Chalk.rgb(0, 143, 255).bold(board.id)} from the database.`);
    ...
    public disconnectBoard(boardId: string): void {
        ...
        BoardsModel.log.debug(
            `Setting board with id ${Chalk.rgb(0, 143, 255).bold(boardId)}'s status to disconnected.`,
        );
        board.disconnect();
        board.save();
    ...
    public async updateBoard(boardUpdates: IBoard): Promise<void> {
        ...
        BoardsModel.log.debug(
            `Storing update for board with id ${Chalk.rgb(0, 143, 255).bold(board.id)} in the database.`,
        );
    ...
    private async createAndPersistBoard(board: Board): Promise<Board> {
        return new Promise(async (resolve, reject) => {
            BoardsModel.log.debug(
                `Storing new board with id ${Chalk.rgb(0, 143, 255).bold(board.id)} in the database.`,
            );
